...
def build_model(trainX,
    ...
    for i in range(0, task_num):
        locals()['input' + str(i)] = Input(shape=(trainX[i].shape[1], trainX[i].shape[2]),
            name='input' + str(i))
        locals()['lstm_out' + str(i)] = Dense(16, activation='relu')(locals()['input' + str(i)])
    ...
    output_list = []
    for i in range(0, task_num):
        locals()['sub' + str(i)] = Dense(16, activation='relu')(concate_layer)
        locals()['sub' + str(i)] = Flatten()(locals()['sub' + str(i)])
        locals()['out' + str(i)] = Dense(6, activation='sigmoid')(locals()['sub' + str(i)])
...
def main():
    ...
    file_list_test = glob.glob('preprocessed_data/test/*.csv')  # print (file_list[0])
    ...
    file = open('time_cost/M_MLP.txt', 'w')
