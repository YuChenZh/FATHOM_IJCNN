...
import glob
import sys
sys.path.insert(0, '/Users/yujingchen/PycharmProjects/WATCH_proj/weather_prediction/')
import helper_funcs
...
def main():
    ...
    testX_list = []
    testy_list = []
    file_list_train = glob.glob('../preprocessed_data/train/attplot_train/*.csv')
    ...
    for i in range(len(file_list_test)):
        ...
        locals()['dataset_test' + str(i)], locals()['scaled_test' + str(i)], locals()[
            'scaler_test' + str(i)] = helper_funcs.load_dataset(locals()['dataset_test' + str(i)])
    ...
    for j in range(10):
        attention_vector = np.mean(helper_funcs.get_activationsD(model,testX_list[0][0:240,:,:],
    ...
    attention_vector_final = np.delete(attention_vector_final, np.s_[9:], axis=1)
