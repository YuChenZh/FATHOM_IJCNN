...
def main():
    ...
    file = open('time_cost/Single_LSTM_1.txt', 'w')
    sum_Smape = 0
    sum_Smape_speed = 0
    ...
    for i in range(len(file_list_train)):
        ...
        # fit network
        history = model.fit(locals()['train_X' + str(i)], locals()['train_y' + str(i)], epochs=100, batch_size=100, validation_data=(locals()['test_X' + str(i)], locals()['test_y' + str(i)]), verbose=2,
                            shuffle=False,
                            callbacks=[
